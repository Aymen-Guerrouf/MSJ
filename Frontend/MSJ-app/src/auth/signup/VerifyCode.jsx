// src/auth/forgot/components/VerifyCode.jsx
...
export default function VerifyCode() {
  ...
  const setAt = (idx, v) => {
    ...
    next[idx] = n;
    setDigits(next);
    if (n && idx < 5) refs[idx + 1].current?.focus();
  };
  ...
  const submitCode = async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch(API_ENDPOINTS.AUTH.VERIFY_RESET_CODE, {
        method: "POST",
  ...
  const resend = async () => {
    setServerError(null);
    try {
      await fetch(API_ENDPOINTS.AUTH.FORGOT_PASSWORD_REQUEST, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
